...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.row == [routeArray count]){
        ...
        return cell;
    }else if (tabView.segmentIndex == 0) {
        static NSString *CellIdentifier = @"Cell";
        FollowTableCell* cell = (FollowTableCell*) [tableView dequeueReusableCellWithIdentifier:CellIdentifier]; 
        if (cell == nil) {
            NSArray *topLevelObjects = [[NSBundle mainBundle] loadNibNamed:@"FollowTableCell" owner:nil options:nil];
            for(id currentObject in topLevelObjects){
                if([currentObject isKindOfClass:[UITableViewCell class]]){
                    cell = (FollowTableCell*)currentObject;
